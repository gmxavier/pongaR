#' @title Lamb wave dispersion function for antisymmetric modes.
#' @description A utility function that calculates the Lamb wave dispersion function for antisymmetric modes according
...
#' @return The value of the Lamb wave dispersion function for antisymmetric modes according
#'         Wilcox's PhD thesis eq. 2.15, p. 28.
# @details DETAILS
# @examples
# \dontrun{
...
delta_A <- function(x, par) {
  ...
  # Wilcox's PhD thesis eq. 2.15, p. 28
  delta_A <- (k^2 + q_t^2)^2*sinh(q_l*d/2)*cosh(q_t*d/2) -
    4*k^2*q_t*q_l*sinh(q_t*d/2)*cosh(q_l*d/2)
}
